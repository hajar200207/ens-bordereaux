                ...
                <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
                    ...
                    <div class="space-y-4 text-sm">
                        <div>
                            ...
                            <p class="font-semibold text-slate-700"><?php echo e($user->username ?? '-'); ?></p>
                        </div>

                        <div>
                            <p class="text-slate-400">Service</p>
                            <p class="font-semibold text-slate-700"><?php echo e($user->service?->name ?? '-'); ?></p>
                        </div>

                        <div>
                            <p class="text-slate-400">Rôle(s)</p>
                            <p class="font-semibold text-slate-700"><?php echo e($user->roles->pluck('name')->join(', ') ?: '-'); ?></p>
                        </div>
                ...
                <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="rounded-2xl bg-slate-50 p-4">
                            ...
                            <p class="font-semibold text-slate-700 mt-1"><?php echo e($user->first_name ?? '-'); ?></p>
                        </div>

                        <div class="rounded-2xl bg-slate-50 p-4">
                            <p class="text-sm text-slate-400">Nom</p>
                            <p class="font-semibold text-slate-700 mt-1"><?php echo e($user->last_name ?? '-'); ?></p>
                        </div>

                        <div class="rounded-2xl bg-slate-50 p-4">
                            <p class="text-sm text-slate-400">Email</p>
                            <p class="font-semibold text-slate-700 mt-1"><?php echo e($user->email); ?></p>
                        </div>

                        <div class="rounded-2xl bg-slate-50 p-4">
                            <p class="text-sm text-slate-400">Username</p>
                            <p class="font-semibold text-slate-700 mt-1"><?php echo e($user->username ?? '-'); ?></p>
                        </div>

                        <div class="rounded-2xl bg-slate-50 p-4">
                            <p class="text-sm text-slate-400">Date de création</p>
                            <p class="font-semibold text-slate-700 mt-1"><?php echo e($user->created_at?->format('d/m/Y H:i') ?? '-'); ?></p>
                        </div>

                        <div class="rounded-2xl bg-slate-50 p-4">
                            <p class="text-sm text-slate-400">Dernière mise à jour</p>
                            <p class="font-semibold text-slate-700 mt-1"><?php echo e($user->updated_at?->format('d/m/Y H:i') ?? '-'); ?></p>
                        </div>
                    </div>

                    <div class="mt-6 flex justify-end gap-3">
                        <form action="<?php echo e(route('admin.users.destroy', $user)); ?>" method="POST" onsubmit="return confirm('Supprimer cet utilisateur ?')">
                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\Lenovo\Desktop\ens-bordereaux\ens-bordereaux\resources\views/admin/users/show.blade.php ENDPATH**/ ?>